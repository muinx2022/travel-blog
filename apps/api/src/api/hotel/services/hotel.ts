...
export default factories.createCoreService(UID, ({ strapi }) => ({
    ...
    const enriched = await Promise.all(
      (data as any[]).map(async (item) => {
        ...
        return {
          ...
          roomTypesCount: Array.isArray(item.roomTypes) ? item.roomTypes.length : 0,
        };
      }),
    );
  ...
  async findOneForAdmin(documentId: string, query: any) {
    return strapi.documents(UID).findOne({
      documentId,
      populate: query?.populate ?? ['categories', 'author', 'amenities', 'roomTypes', 'thumbnail', 'images'],
      fields: query?.fields,
