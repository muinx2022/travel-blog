...
export default function NewMyPostPage() {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      const res = await fetch("/api/my-posts-proxy", {
        ...
        body: JSON.stringify({ title, content, categories: selectedCategoryDocumentIds }),
      });
      ...
      if (payload.data?.documentId && mediaPendingId) {
        await bulkUpdateEntityDocumentId(mediaPendingId, payload.data.documentId, jwt ?? undefined);
      }
  ...
  return (
    <ServiceFormLayout
      ...
      title="Tạo bài viết"
      subtitle="Viết và đăng bản nháp bài viết mới của bạn."
    >
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <ServiceFormSection icon={<Images className="h-4 w-4" />} title="Hình ảnh" bodyClassName="space-y-6 p-6">
          <div className="rounded-xl border border-zinc-100 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-800/30">
            <EntityMediaManager entityType="post" entityDocumentId={mediaPendingId} category="thumbnail" multiple={false} label="Ảnh đại diện" />
          </div>
          <div className="rounded-xl border border-zinc-100 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-800/30">
            <EntityMediaManager entityType="post" entityDocumentId={mediaPendingId} category="gallery" multiple={true} maxFiles={20} label="Thư viện ảnh" />
          </div>
        </ServiceFormSection>
