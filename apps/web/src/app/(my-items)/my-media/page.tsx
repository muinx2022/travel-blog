...
export default function MyMediaPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      query.set("filters[author][id][$eq]", String(user.id));
      ...
      <div className="mx-auto max-w-4xl px-4 py-12">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50">
            Đăng nhập để xem media của bạn
          </h1>
          <Link
            href="/login"
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            Quản lý tất cả ảnh và video bạn đã upload
          </p>
        </div>
        <button
          type="button"
      ...
      {/* Filters */}
      <div className="mb-6 flex flex-wrap items-center gap-3 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
      ...
      {!loading && medias.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-zinc-200 bg-zinc-50 py-16 dark:border-zinc-800 dark:bg-zinc-900/50">
          ...
          <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-500">
            Tải ảnh lên khi tạo bài viết, tour, hoặc khách sạn
          </p>
        </div>
      )}
