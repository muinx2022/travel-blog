...
export default factories.createCoreController('api::hotel.hotel', ({ strapi }) => ({
  ...
  async findOne(ctx) {
    ...
    try {
      ...
      console.error('[hotel.findOne] error:', error);
      return ctx.notFound('Hotel not found');
    }
  },
  ...
  async userCreate(ctx) {
    ...
    try {
      ...
      console.error('[hotel.userCreate] error:', error);
      return ctx.badRequest('Failed to create hotel');
    }
  },
  ...
  async userUpdate(ctx) {
    ...
    try {
      const existing = await strapi.documents('api::hotel.hotel').findOne({
        documentId,
        populate: ['author'],
      });

      if (!existing || existing.author?.id !== userId) {
        return ctx.forbidden('You can only update your own hotels');
      }
      ...
      const updated = await strapi.documents('api::hotel.hotel').update({
        documentId,
        data: {
          ...(title && { title }),
          ...(content && { content }),
      ...
      console.error('[hotel.userUpdate] error:', error);
      return ctx.badRequest('Failed to update hotel');
    }
  },
  ...
  async myHotels(ctx) {
    ...
    try {
      const [hotels, total] = await Promise.all([
        strapi.documents('api::hotel.hotel').findMany({
          filters: {
            author: {
              id: userId,
            },
        ...
        strapi.documents('api::hotel.hotel').count({
          filters: {
            author: {
              id: userId,
            },
      ...
      console.error('[hotel.myHotels] error:', error);
      ctx.body = {
        data: [],
        meta: {
  ...
  async userPublish(ctx) {
    ...
    try {
      ...
      if (!existing || existing.author?.id !== userId) {
        return ctx.forbidden('You can only publish your own hotels');
      }

      const documentsApi = strapi.documents('api::hotel.hotel') as any;
      ...
      if (!existing.id) {
        return ctx.badRequest('Hotel not found');
      }

      await strapi.entityService.update('api::hotel.hotel', existing.id, {
        data: { publishedAt: new Date().toISOString() },
      });

      const updated = await strapi.documents('api::hotel.hotel').findOne({
        documentId,
        status: 'published',
      });
      ...
      console.error('[hotel.userPublish] error:', error);
      return ctx.badRequest('Failed to publish hotel');
    }
  },
  ...
  async userUnpublish(ctx) {
    ...
    try {
      ...
      if (!existing || existing.author?.id !== userId) {
        return ctx.forbidden('You can only unpublish your own hotels');
      }
      ...
      console.error('[hotel.userUnpublish] error:', error);
      return ctx.badRequest('Failed to unpublish hotel');
    }
  },
}));
