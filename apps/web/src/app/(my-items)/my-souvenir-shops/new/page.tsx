...
export default function NewMySouvenirShopPage() {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      const res = await fetch("/api/my-souvenir-shops-proxy", {
        ...
        body: JSON.stringify({ title, content, categories: selectedCategoryDocumentIds, address, city, phone }),
      });
      const payload = (await res.json().catch(() => ({}))) as { data?: { documentId?: string }; error?: string };
      if (!res.ok) throw new Error(payload.error || "Tạo cửa hàng lưu niệm thất bại");
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "Tạo cửa hàng lưu niệm thất bại"); }
  ...
  return (
    <ServiceFormLayout
      ...
      title="Tạo cửa hàng lưu niệm"
      subtitle="Thêm cửa hàng lưu niệm mới của bạn."
    >
      <form onSubmit={onSubmit} className="space-y-6">
        <ServiceFormSection icon={<Type className="h-4 w-4" />} title="Thông tin cơ bản">
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Tên cửa hàng <span className="text-red-500">*</span></label>
            <input value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="Nhập tên cửa hàng..." className={inputCls.replace("text-sm", "text-base")} />
          </div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="space-y-2"><label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Thành phố</label><input value={city} onChange={(e) => setCity(e.target.value)} placeholder="VD: Hội An" className={inputCls} /></div>
            <div className="space-y-2"><label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Số điện thoại</label><input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="VD: 0901234567" className={inputCls} /></div>
          </div>
          <div className="space-y-2">
            ...
            {loadingCategories ? (
              <div className="flex items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-700 dark:bg-zinc-800/50"><div className="h-4 w-4 animate-spin rounded-full border-2 border-zinc-300 border-t-violet-500"></div><span className="text-sm text-zinc-500 dark:text-zinc-400">Đang tải danh mục...</span></div>
            ) : (
        ...
        <ServiceFormSection icon={<FileText className="h-4 w-4" />} title="Nội dung" bodyClassName="p-6"
          headerRight={<button type="button" onClick={() => setShowToolbar((v) => !v)} className="rounded-lg px-3 py-1.5 text-xs font-medium text-violet-600 transition-colors hover:bg-violet-50 hover:text-violet-700 dark:text-violet-400 dark:hover:bg-violet-900/20">{showToolbar ? "Ẩn thanh công cụ" : "Hiển thị thanh công cụ"}</button>}
        >
        ...
        <ServiceFormSection icon={<Images className="h-4 w-4" />} title="Hình ảnh" bodyClassName="space-y-6 p-6">
          <div className="rounded-xl border border-zinc-100 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-800/30">
            <EntityMediaManager entityType="souvenir-shop" entityDocumentId={mediaPendingId} category="thumbnail" multiple={false} label="Ảnh đại diện" />
          </div>
          <div className="rounded-xl border border-zinc-100 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-800/30">
            <EntityMediaManager entityType="souvenir-shop" entityDocumentId={mediaPendingId} category="gallery" multiple={true} maxFiles={20} label="Thư viện ảnh" />
          </div>
        </ServiceFormSection>
        ...
        <div className="flex items-center justify-end gap-3 pt-4">
          ...
          <button type="submit" disabled={submitting} className="rounded-xl bg-gradient-to-r from-violet-600 to-violet-500 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-violet-500/25 transition-all duration-200 hover:from-violet-700 hover:to-violet-600 focus:outline-none focus:ring-4 focus:ring-violet-500/20 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60">
            {submitting ? <span className="flex items-center gap-2"><span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white"></span>Đang tạo...</span> : "Tạo cửa hàng"}
          </button>
        </div>
      </form>
    </ServiceFormLayout>
