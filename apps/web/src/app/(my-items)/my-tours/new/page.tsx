...
export default function NewMyTourPage() {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      const res = await fetch("/api/my-tours-proxy", {
        ...
        body: JSON.stringify({ title, content, categories: selectedCategoryDocumentIds, destination, duration: duration ? Number(duration) : undefined, price: price ? Number(price) : undefined, itinerary: itinerary.filter((d) => d.title.trim()) }),
      });
      const payload = (await res.json().catch(() => ({}))) as { data?: { documentId?: string }; error?: string };
      if (!res.ok) throw new Error(payload.error || "Tạo tour thất bại");
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "Tạo tour thất bại"); }
  ...
  return (
    <ServiceFormLayout
      ...
      subtitle="Thêm tour du lịch mới của bạn."
    >
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <ServiceFormSection icon={<Images className="h-4 w-4" />} title="Hình ảnh" bodyClassName="space-y-6 p-6">
          <div className="rounded-xl border border-zinc-100 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-800/30">
            <EntityMediaManager entityType="tour" entityDocumentId={mediaPendingId} category="thumbnail" multiple={false} label="Ảnh đại diện" />
          </div>
          <div className="rounded-xl border border-zinc-100 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-800/30">
            <EntityMediaManager entityType="tour" entityDocumentId={mediaPendingId} category="gallery" multiple={true} maxFiles={20} label="Thư viện ảnh" />
          </div>
        </ServiceFormSection>
        ...
        <div className="flex items-center justify-end gap-3 pt-4">
          ...
          <button type="submit" disabled={submitting} className="rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-500 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 transition-all duration-200 hover:from-emerald-700 hover:to-emerald-600 focus:outline-none focus:ring-4 focus:ring-emerald-500/20 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60">
            {submitting ? <span className="flex items-center gap-2"><span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white"></span>Đang tạo...</span> : "Tạo tour"}
          </button>
        </div>
      </form>
    </ServiceFormLayout>
